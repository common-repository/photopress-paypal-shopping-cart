...
class papt_spsc {
	...
	static function outputFormHandlerJs() {
		...
		var photopressShoppingCart = {
			...
			updateFields : function( selected ) {
				...
				jQuery("#photopress-spsc-hash_two").val( selected.attr("hash_two") );
	...
	static function generateForm ($product_name, $variations = array(), $shipping = 0, $echo = false ) {
		...
		foreach ( $variations as $category => $variation ) {
			...
			foreach ($variation as $v) {
				...
				$options .= sprintf(
					...
					'<option value="%s" product_name= "%s" variation_name="%s" price="%s" shipping="%s" hash_one="%s" hash_two="%s">%s</option>', 
					$v['name']. ': '.$v['price'], 
					$product_name,
					$v['name'],
					$v['price'], 
        ...
        $form .= sprintf('<input id="photopress-spsc-hash_two" type="hidden" name="hash_two" value="%s" />', $hash_two );
		$form .= sprintf('<input type="hidden" name="cartLink" value="%s" />', cart_current_page_url() );
		$form .= '<input type="hidden" name="addcart" value="1" />';
		$form .= wp_nonce_field( 'wspsc_addcart', '_wpnonce', true, false );
		$form .= '</form>';
		$form .= '</div>';
